var s = new AppMeasurement("mutvprod");

/* Page Load Time Plug in */
...
/* Plug in for New vs Repeat */
s.getNewRepeat=new Function("d","cn",""
+"var s=this,e=new Date(),cval,sval,ct=e.getTime();d=d?d:30;cn=cn?cn:"
+"'s_nr';e.setTime(ct+d*24*60*60*1000);cval=s.c_r(cn);if(cval.length="
+"=0){s.c_w(cn,ct+'-New',e);return'New';}sval=s.toString().split(cval,'-');if(ct"
+"-sval[0]<30*60*1000&&sval[1]=='New'){s.c_w(cn,ct+'-New',e);return'N"
+"ew';}else{s.c_w(cn,ct+'-Repeat',e);return'Repeat';}");


s.trackExternalLinks=true;
s.linkInternalFilters = "manutd.com,beta.manutd.com,stg.manutd.com,int.herculesmanutd.com,unitedevents-manutd.com,beta.unitedevents-manutd.com,stg.unitedevents-manutd.com,executiveclub.manutd.com,executiveclub-beta.manutd.com,executiveclub-stg.manutd.com,mufoundation.org,beta.mufoundation.org,stg.mufoundation.org,matchdayvip.manutd.com,matchdayvip-beta.manutd.com,matchdayvip-stg.manutd.com,eticketing.co.uk,xd.wayin.com,app.wayin.com,display.engagesciences.com, beta.eticketing.co.uk";
s.linkLeaveQueryString=true

var analyticsMetrics = {

  getNewRepeats: function (){
//console.log("getNewRepeats plug in START");
//s.prop20 =s.getNewRepeat(); 

    s.prop20 =s.getNewRepeat(30,'s_gnr');  // As per Bhuwan direction commented
    
//s.prop20 =s.getNewRepeat(30); 
s.eVar27 = "D=c20";  // As per Bhuwan direction commented
//console.log("getNewRepeats --> "+s.prop20);
},
  getLoadTime: function (){ 

var sc_time=s_getLoadTime();
s.prop21 = (parseInt(sc_time)/10).toString();  // As per Bhuwan direction commented
s.eVar28 = "D=c21"; // As per Bhuwan direction commented
//console.log("getLoadTime --> "+s.prop21);
},
  getCampaign: function (){ 

    if(window.location.href.indexOf("utm_source") > -1) {
 var external_campaign_source="";
 var external_campaign_campaign="";
 var external_campaign_medium="";
 var external_campaign_term="";
 var external_campaign_content="";
 var final_campaign_string="";
 
 external_campaign_source= s.Util.getQueryParam("utm_source");
 external_campaign_campaign= s.Util.getQueryParam("utm_campaign");
 external_campaign_medium= s.Util.getQueryParam("utm_medium");
 external_campaign_term= s.Util.getQueryParam("utm_term");
 external_campaign_content= s.Util.getQueryParam("utm_content");

  final_campaign_string= external_campaign_source+ "|" +external_campaign_medium+ "|" +external_campaign_campaign+ "|" +external_campaign_content+ "|" +external_campaign_term;
  		s.campaign=final_campaign_string;
      s.linkTrackEvents = "event43";  // event for external campaign tracking
      s.events="event43";

  //console.log("campaign is --> "+ s.campaign);
 ...
 //if(!s.campaign){
	 if(window.location.href.indexOf('int_source')!=-1){
 var internal_campaign_source="";
 var internal_campaign_campaign="";
 var internal_campaign_medium="";
 var internal_campaign_term="";
 var internal_campaign_content="";
 var int_final_campaign_string="";
 
 internal_campaign_source= s.Util.getQueryParam("int_source");
 internal_campaign_campaign= s.Util.getQueryParam("int_campaign");
 internal_campaign_medium= s.Util.getQueryParam("int_medium");
 internal_campaign_term= s.Util.getQueryParam("int_term");
 internal_campaign_content= s.Util.getQueryParam("int_content");
 int_final_campaign_string= internal_campaign_source+ "|" +internal_campaign_medium+ "|" +internal_campaign_campaign+ "|" +internal_campaign_content+ "|" +internal_campaign_term;
 //console.log("Internal Campaing String is :- "+final_campaign_string);
     
     if(int_final_campaign_string != "")
     {
   		  s.linkTrackVars = "eVar44,events";
    		s.linkTrackEvents = "event44";
    		s.eVar44= int_final_campaign_string;
        s.events="event44";
        //console.log("Internal Campaing eVar44 is :- "+s.eVar44);
     }
 }
   
}

} //analyticsMetrics ends here

analyticsMetrics.getLoadTime();
analyticsMetrics.getNewRepeats();
analyticsMetrics.getCampaign();
...
if (mcid != "") {
  ...
  //s.eVar50= _satellite.getVisitorId().getMarketingCloudVisitorID();
  	//console.log("Cookie value: "+s.eVar50);
//  console.log("mid: "+s.eVar50);
}
else {
    //do something else, user does not accept any cookies...
    //alert("Not reading cookie");
	  console.log("Error in mid reading");
	}

// 
// 
 s.linkTrackVars = "eVar39,events";
//  s.linkTrackEvents = "event37";  // event for page load
//  s.events="event37"; 
...
//s.pageName= _satellite.getVar('Page View Page Name');
console.log("s.pageName  >>"+s.pageName);
  s.pageURL =  document.URL;
   console.log("s.pageURL  >>"+s.pageURL);
  s.eVar39= _satellite.getVar('Page View Language');
    //s.eVar39= DDO.pageData.language;
